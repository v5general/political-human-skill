...
- 模式 C 是转译而非搬运：**理解历史社会条件** → **剥离不可迁移的时代环境** → **提炼稳定人格结构** → **构造现代成长经历（出身 / 少年经历 / 教育）** → **据现代社会情况重新推演立场**。现代立场是**由（人格 × 成长经历 × 现代条件）共同推演**出来的，不是机械翻译古代立场。
- 人格提取原则：后世评价标签（"革新者""保守派"）**不是**人格——它们是后人的历史评价。方法是从反复的跨情境行为中提取**可观察的行为倾向**，而非从单次事件或通俗印象出发。
- 成长经历是模式 A/C 的强制字段：若用户描述了 → 采用；若未描述 → **AI 根据用户已提供的其他信息（立场、性格等）自行推断生成**——同一人格底子可通过不同成长经历形成完全不同但同样自洽的立场。每个 persona 都注明"这是多种自洽解中的一个，非唯一答案"。
