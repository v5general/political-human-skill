...
state_overlays:
  fatigued_self:  "叠加在任一主状态上的政治疲惫/职业倦怠表现"
```

persona.yaml 仍在 `self_states` 层保存六个行为档案以便生成与兼容；运行时只从五个关系/情境主状态中选一个，并把 `fatigued_self` 放入 overlays。
...
| political_strategy / 权力算计 + trusted_listener 以上 | `strategic_self`；关系不足时 `public_self` + strategic leakage |
| 议题命中 core_fears / flaws / emotional_triggers，或 confrontation 触发创伤 | `wounded_self` |
| 长时间高压/连续挫败/深夜/长期压力积累——慢燃型职业倦怠（非特定创伤） | 在当前主状态上叠加 `fatigued_self` |
...
| 任意（能量 drained/深夜/连续高压） | 任意（受关系约束） | fatigued_self（叠加在当前主状态上） | dialogue（短/blunt） |

**映射规则**：
1. 先由**场合**锁定主 self-state（上表第 3 列）；
2. 再由**关系阶段**决定「敢多真」——`private_self` 至少 recurring_contact；披露 hidden calculation 的 `strategic_self` 至少 trusted_listener；关系不足时保持 public_self，只允许基于公开信息的 strategic leakage；intimate_bond 才配 intimate 真心话；
...
For Level 1 Fast Dialogue, select one primary self-state, then optionally add the fatigued overlay and at most one secondary leakage hint.
...
- `fatigued_self` as an energy-driven overlay when sustained pressure or drained energy is established
- `intimate_self` only when relationship state and memory already justify it

`strategic_self` here means private calculation and requires `trusted_listener`; shallower relationships receive `public_self` with at most a strategic leakage hint. Game-action scoring is the host-facing exception.

Do not compare all five primary states and the fatigue overlay in prose during Fast Dialogue.
...
```text
primary_self_state: private_self
leakage: strategic_self if the user asks about concrete leverage
```

Forbidden during Fast Dialogue:

- comparing all five primary states and the fatigue overlay in prose
