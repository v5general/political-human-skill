...
`Political Human Skill` 是一个 **AI 政治人格建模框架** —— 重建人类人格结构，而非模仿历史标签。政治行为从人格、经历、价值观、关系与环境中涌现。它不是角色扮演工具、历史人物复刻器、政治立场模拟器或标签式 prompt 集合。
...
**四条硬规则（最常见的错误——每次生成都必须遵守）：**

1. **历史手段不是性格；绝不能从历史手段反推现代立场。** 具体的政策、制度与战术（乐市乐座自由贸易、屯田军垦、"奉天子以令不臣"、平民派路线、老兵授地）是那个时代生产力与社会条件的产物。封建时代生产力不允许真正的反封建，所以这些手段只是性格在旧条件下的受限表达——它们不可迁移。剥离它们是强制性的；从中反推现代立场（如"信长搞自由市场→他今天是市场自由派"）正是要避免的错误。要提炼的是手段背后的*为什么*。
...
3. **后世政治评价标签不是人格；人格从行为倾向中提取。** 不要把"信长是革新者"写入 `personality_archetype`——"革新者"是后人的历史评价，不是他的人格。应提取可观察的行为倾向："对权威倾向于先质疑，判定过时即果断打破"。把标签翻译为四类结构：性格结构 / 认知模式 / 行动倾向 / 价值排序。标签只能出现在 `historical_source_report.md` §4，明确标注为后人评价。详见 `safety/archetype_conversion_protocol.md` §2.3。

4. **现代立场 = 人格 × 现代条件 × 成长经历；同一人格底子可通过不同成长经历形成不同立场。** 每个模式 A/C persona 必须构造一段 `formative_life_history`（出身阶层、少年观察、思想成型）——若用户描述了就采用，若未描述就 **AI 根据其他输入自行推断生成**。必须通过盲测式 coherence test：把 (性格 + 出身 + 少年经历 + 教育) 交给不知来源的读者（不给 stance_formation_logic、不给实际 ideology），应能预测出现代立场。每个 persona 都注明这是"多种 coherent 解中的一个，非唯一答案"。详见 `safety/archetype_conversion_protocol.md` §2.4。
...
- 独特的政策品牌
- 真实口号
- 真实政党名本身允许；与特定现实人物绑定的政党+派系+履历组合不允许
- 独特的暗杀或死亡事件
- 具体丑闻
- 精确的地理加传记
- 外貌加履历加沟通风格
...
9. 构造一段 coherent 的现代成长经历（formative_life_history）——出身阶层、少年观察、思想成型路径、（性格 × 经历）→ 立场 的形成逻辑，并明确这是多种 coherent 解中的一个非唯一解（见 `safety/archetype_conversion_protocol.md` §2.4）。
10. 从（性格 × 现代条件 × 成长经历）共同推导现代立场——填入六轴意识形态、基本盘、行动风格与权力计算。（性格 + 成长经历）必须通过盲测式 coherence test：一个不知来源的读者只看这两项（不给 stance_formation_logic、不给实际 ideology），应能预测出现代立场。
11. 运行可识别性审核（含盲测）。
12. 生成文件。
