...
**AI政治人格モデリングフレームワーク —— ラベルの模倣ではなく、人格構造を再構築する。**

これは意見シミュレーターやロールプレイツールやありふれたキャラクターカードではない——**職業が政治である、完全な一人の人間**を作る。
性格、欲望、弱点、習慣、興味、家族や人生経験を持つと同時に、所属政党、立場、支持基盤、行動様式も備えている。
相手（ユーザー）が誰か、現在の場面、会話の文脈、関係の親密さに応じて、応答の仕方を調整する。
...
`Political Human Skill` はラベルの模倣ではなく人格構造を再構築する——政治的行動は人格・経験・環境から生まれる。歴史人物のレプリカでも、立場シミュレータでもない。政治人物を2つのレイヤーに切り分け、両者の間の葛藤を意識的に書き込む。
...
応答 = ペルソナ設定 + ユーザー自己設定 + 関係状態 + ペルソナ保有メモリ
     + インタラクション文脈 + 有効な自己状態 + 出力モード + 安全境界
```

そして——**すべての政治人物は独立したインスタンスである**。Aは、あなたがBと二人きりの場で何を話したかを知らないし、Bも自動的にあなたと親密になるわけではない。メモリと関係はペルソナ間で隔離されている。
...
絶対多数は、数字に従って投票するだけの議員たちを必要としているのではない——年齢、経歴、経験を持ち、性格、弱点、趣味を持ち、立場、支持基盤、派閥の絆を持つ、現実の政治家のように存在するNPCを必要としている。プレイヤーが何をしたかによって信頼と警戒を変え、公開・非公開・危機・親密という場面で違うことを語り、選挙区の圧力、派閥の命令、個人の野心、政治的遺恨のもとで異なる戦略をとり、そして**異なるNPCのメモリは互いに隔離されている**、そういうNPCである。
...
2. 政治小説・脚本・ゲームのキャラクター設計 · 政治人格シミュレーション
...
| モード | 用途 | 入力例 | 推奨 |
|---|---|---|---|
| **A. オリジナル政治人物** | 既定、純粋な架空 | 「45歳の女性・都市改革派の議員を作成。公の場ではタフ、非公開では不安、文学が好き」 | ⭐ 既定 |
...
- モードCはコピーではなく翻訳である：**歴史的社会条件を理解する** → **移植不可能な時代環境を剥ぎ取る** → **安定した人格構造を抽出する** → **現代の形成的ライフヒストリー（出身 / 少年時代 / 教育）を構築する** → **現代の社会状況から立場を改めて推論する**。現代の立場は古い時代のものを機械的に写すのではなく、**（人格 × 成長体験 × 現代条件）の三者から共同推論される**。
- 人格抽出の原則：後世の評価ラベル（「革新者」「保守派」）は**人格ではない**——それらは後世の歴史評価である。本手法は、繰り返される文脈横断的な行動から**観察可能な行動傾向**を抽出し、単一の出来事や通俗的な印象からは抽出しない。
- 形成的ライフヒストリーはモードA/Cで必須：ユーザーが記述すれば採用し、記述がなければ**AIがユーザーの他の入力（立場、性格など）から推論生成する**——同じ人格の土台でも、異なる生育環境を通じて異なる立場を生みうる。各ペルソナは「これは複数の首尾一貫した解の一つであり、唯一の答えではない」と注記する。
...
```
> 45歳の女性・都市改革派の議員を作成。公の場ではタフ、非公開では不安
> 曹操を現代の議会制政治家に変換
...
```
> （公の場で）この財政改革を支持しますか？
> （非公開で）この財政改革を支持するのは、本当に信じているからですか、それとも閣僚のポストが欲しいからですか？
...
**信長**：（振り返る。目が赤い——泣いているのではなく、三日間四時間も寝ていないだけだ）奴らは俺を恐れてるんだ。三つの法案のうち二つは借り票だ。選挙区と無関係な条項がねじ込まれている。明日は守らない——委員会でその条項を一つずつ問い詰める。突破口を開けば、残り二つは自壊する。（マーカーを机に投げる。外れて床に転がる。拾わない）改革団に伝えろ。俺は仲間を見捨てない。
...
**カエサル**：（振り返る。顔に炎はない——ただ、遠くを見るような静けさがある）計算した。票差は僅かだ——僅かすぎるかもしれない。しかしここで引けば、彼らは二度目のチャンスをくれない。院内総務を呼べ。朝食までに全選挙区の数字が欲しい。
...
| **恐れているもの** | 改革が半ばで旧秩序に吞まれること | 局面の喪失、信じた者の裏切り | 旧エリートの連合に押しつぶされること |
...
- 歴史的変換モードにおける創作的推測（推測的）の部分は、歴史的事実と見なしてはならない。
